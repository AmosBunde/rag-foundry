...
# Filename / media safety heuristics
...
def guard_query(text: str, max_length: int = 2_000) -> None:
    ...
    result = validate_query(text, max_length, use_presidio=settings.use_presidio)
...
def validate_media_metadata(filename: str | None, content_type: str | None, size_bytes: int, max_mb: int) -> GuardrailResult:
    """Validate uploaded media file metadata for safety and size."""
    violations: list[str] = []
    reasons: list[str] = []

    if not filename or _UNSAFE_FILENAME_PATTERN.search(filename):
        violations.append("unsafe_filename")
        reasons.append("Filename contains unsafe characters")

    if size_bytes > max_mb * 1024 * 1024:
        violations.append("file_too_large")
        reasons.append(f"File exceeds maximum size of {max_mb} MB")

    if not content_type:
        violations.append("missing_content_type")
        reasons.append("Content-Type header is required")
...
def guard_media(filename: str | None, content_type: str | None, size_bytes: int, max_mb: int) -> None:
    """Raise HTTPException if media guardrails block the upload."""
    result = validate_media_metadata(filename, content_type, size_bytes, max_mb)
    if not result.allowed:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"error": "Media guardrail violation", "reason": result.reason, "violations": result.violations},
        )
