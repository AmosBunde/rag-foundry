...
variable "domain" {
  description = "Public domain name for the Hybrid RAG API"
  type        = string
}
...
variable "app_image" {
  description = "Docker image for the Hybrid RAG FastAPI backend"
  type        = string
  default     = "rag-foundry/hybrid-rag-backend:latest"
}
