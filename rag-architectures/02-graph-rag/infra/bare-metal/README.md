# Graph RAG — Bare-Metal / VM Deployment

This module generates Docker Compose, Nginx, systemd, and deployment scripts for running the Graph RAG stack on a single bare-metal or VM host.

## What's included

- `docker-compose.yml` — FastAPI backend + Postgres + Redis + Qdrant + Neo4j + optional Ollama
- `nginx.conf` — Reverse proxy to the FastAPI service on port 8002
...
terraform apply -var="domain=rag.example.com" \
                ...
                -var="neo4j_password=$(openssl rand -base64 32)" \
...
| `app_image` | Backend Docker image | `rag-foundry/graph-rag-backend:latest` |
| `neo4j_password` | Neo4j password | `ragfoundry` |
...
- All persistent data uses named Docker volumes. Back them up with `docker run --rm -v graph-rag_postgres_data:/data -v $(pwd):/backup alpine tar czf /backup/postgres.tgz -C /data .`.
