...
scp -i "$${KEY}" -o StrictHostKeyChecking=no -r \
  ...
  out/nginx.conf \
  out/.env \
  "$${USER}@$${HOST}:/tmp/agentic-rag-hospital-deploy/"

ssh -i "$${KEY}" -o StrictHostKeyChecking=no "$${USER}@$${HOST}" "
  set -e
  sudo mv /tmp/agentic-rag-hospital-deploy/* ${deploy_path}/
...
%{ endif ~}
  ...
  sudo cp ${deploy_path}/agentic-rag-hospital-api.service /etc/systemd/system/
  sudo cp ${deploy_path}/agentic-rag-hospital-nginx.service /etc/systemd/system/
  sudo systemctl daemon-reload
  sudo systemctl enable --now agentic-rag-hospital-api agentic-rag-hospital-nginx
"
