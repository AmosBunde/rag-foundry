...
scp -i "$${KEY}" -o StrictHostKeyChecking=no -r \
  ...
  out/nginx.conf \
  out/.env \
  "$${USER}@$${HOST}:/tmp/multimodal-rag-deploy/"

ssh -i "$${KEY}" -o StrictHostKeyChecking=no "$${USER}@$${HOST}" "
  set -e
  sudo mv /tmp/multimodal-rag-deploy/* ${deploy_path}/
...
%{ endif ~}
  ...
  sudo cp ${deploy_path}/multimodal-rag-api.service /etc/systemd/system/
  sudo cp ${deploy_path}/multimodal-rag-nginx.service /etc/systemd/system/
  sudo systemctl daemon-reload
  sudo systemctl enable --now multimodal-rag-api multimodal-rag-nginx
"
