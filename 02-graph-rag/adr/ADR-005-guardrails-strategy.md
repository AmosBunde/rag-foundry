...
1. **Input validation** with Pydantic models (`GraphQueryRequest`, `Document`, `IngestRequest`) enforces type, length, and cardinality constraints.
...
5. **YAML configuration files** in `guardrails/` document intended rules for prompt injection, PII, content safety, rate limits, and entity extraction.
...
- Negative: Rate limits in `guardrails/rate-limit-config.yaml` are configuration intent; slowapi must be pointed at Redis for multi-replica deployments.
