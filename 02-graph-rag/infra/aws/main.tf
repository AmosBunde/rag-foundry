...
locals {
  ...
  qdrant_url        = var.qdrant_endpoint
  neo4j_url         = var.neo4j_endpoint
  ollama_url        = var.ollama_endpoint
  ...
  common_env = {
    ...
    NEO4J_URI            = local.neo4j_url
    OLLAMA_URL           = local.ollama_url
    LLM_MODEL            = "llama3:8b"
    EMBEDDING_MODEL      = "nomic-embed-text"
    GRAPH_COLLECTION     = "graph_rag_chunks"
    DEFAULT_TOP_K        = "5"
    GRAPH_DEPTH          = "2"
  }
...
resource "aws_secretsmanager_secret" "api" {
  name_prefix = "${local.name_prefix}-api-"
  description = "Graph RAG API secrets"
}
...
resource "aws_elasticache_replication_group" "redis" {
  ...
  description                = "Graph RAG Redis cluster"
